...
api_forum_details = function(r)
    if not keys_present(r.query_params, { 'forum' }) then
        return api_response(ResponseCode.MeaninglessRequest, {})
    end

    local forum = conn:execute('select * from forum where short_name = ?', r.query_params.forum)
    if forum then
        return api_response(ResponseCode.Ok, forum)
    end
...
server:hook('before_dispatch', function(self, request)
    ...
    if request.method ~= 'GET' then
        ...
        if not obj.json then
            obj.response = api_response(ResponseCode.BadRequest, err)
        end
    else
